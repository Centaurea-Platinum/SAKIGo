...
## D13 — Stem: small group-equivariant CNN, regular representation (2026-06-17)

A small G-CNN (regular rep) as the stem. **Why:** bakes in Go's D4 board symmetry structurally instead of relying only on data augmentation. **Open:** regular rep multiplies channels by |G| (×8 for D4) — costly, hence "small"; and equivariance is end-to-end-or-nothing (the register-attention trunk isn't D4-equivariant), so either group-pool at the stem boundary or treat it as a strong front-end prior, not a whole-net guarantee. Color-swap / komi / Boundary on non-square boards aren't spatial symmetries and don't apply. Source: [Stem.md](../Design/Architecture/Stem.md).

## D14 — Trunk: KataGo nested residual blocks + register-token attention (2026-06-17)

Conv nested-bottleneck residual blocks (KataGo-style); no spatial self-attention yet; **register tokens** with QKV attention between registers and each spatial token. **Why:** registers are a cheap global-information pathway — O(N·R) vs O(N²) for full spatial attention — a principled alternative to KataGo's global pooling, and they double as the global-head summary (→ D15). FiLM injection sites live here. Source: [Trunk.md](../Design/Architecture/Trunk.md).
...
Two *retained* scalars in [−1, 1], divided by board size; handicap folded into komi implicitly. **Why:** compact and board-size-agnostic; the prisoner count is needed for Territory scoring, so it stays. CapturedStones can exceed board size, but that overflow is deliberately left unhandled (rare, and harmless as a NN input). Source: [NonBoardInput.md](../Design/Input/NonBoardInput.md).
