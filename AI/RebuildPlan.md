...
**Status (2026-07-06, historical plan): P0–P6 completed and the cutover was executed with owner approval.** Legacy `Training/*.py` and `Model/` were deleted; retained `Training/data` and `Training/runs` are reference artifacts. The current implementation and contracts live under `sakigo/`; later direct-book and model-sweep decisions supersede several target and batching details below. This file records the rebuild rationale rather than current operating instructions.
