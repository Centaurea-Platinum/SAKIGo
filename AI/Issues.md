...
- **[Gap] Search undefined.** `../Design/Search/` is empty — the algorithm (MCTS?) and how the budget head's prior feeds it are unspecified. Author has flagged a **PUCT redesign**; candidate directions below.
...
- **[Open] Policy-vs-Budget targets undefined.** AlphaZero fuses prior and improved policy because both equal the MCTS visit distribution. Splitting them needs each target pinned: if policy regresses to argmax-best it becomes a sparse near-one-hot. Specify what each learns and which one PUCT consults. (D5)
