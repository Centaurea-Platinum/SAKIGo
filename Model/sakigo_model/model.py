...
from .d4 import GROUP_SIZE
from .layers import InvariantHead, RegularLift, RegularPointwiseMLP, TrunkBlock


def _scalar_mlp(channels: tuple[int, ...]) -> nn.Sequential:
    if len(channels) < 2:
...
class SakiGoModel(nn.Module):
    """Spec-backed SAKIGo model with explicit D4 regular representations."""
    ...
    def __init__(self, config: SakiGoModelConfig | str | None = None) -> None:
        ...
        self.lift = RegularLift()
        self.stem = RegularPointwiseMLP(self.config.stem_channels)
        ...
        self.budget_pass_head = RegularPointwiseMLP(
            self._head_channels(
        ...
        self.ownership_head = RegularPointwiseMLP(
            self._head_channels(
        ...
        self.policy_head = RegularPointwiseMLP(
            self._head_channels(
        ...
        self.budget_head = RegularPointwiseMLP(
            self._head_channels(
    ...
    def _validate_config(self) -> None:
        if self.config.architecture != "SakiGoModel":
            raise ValueError(f"SakiGoModel cannot build architecture {self.config.architecture!r}")
    ...
    def initial_registers(self, rules: torch.Tensor) -> torch.Tensor:
        ...
        return registers.unsqueeze(-1).expand(-1, -1, -1, GROUP_SIZE).contiguous()
    ...
    def _spatial_logits(self, regular: torch.Tensor) -> torch.Tensor:
        board = self.invariant(regular).squeeze(1)
        return board.reshape(regular.shape[0], regular.shape[-2] * regular.shape[-1])

    def forward(self, board: torch.Tensor, rules: torch.Tensor) -> dict[str, torch.Tensor]:
        if board.dim() != 4:
            raise ValueError("SakiGoModel expects board shape [B,6,N,N]")
